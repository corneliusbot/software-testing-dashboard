"""
Software Spend Shift — Evidence Tracker Dashboard
Hypothesis: Software spend is shifting from building → testing/validation + operations/governance
"""
...
def signal_icon(signal: str) -> str:
    return {"confirms": "🟢", "challenges": "🔴", "neutral": "⚪"}.get(signal, "⚪")
...
DEV_TOOLS_BASKET = {
    ...
    "TEAM": "Atlassian",        # Jira, Bitbucket — planning + build
...
# Updated periodically — used as rough weights, not precision values
APPROX_MARKET_CAPS = {
...
TRENDS_GROUP_C = ["Claude Code", "ChatGPT Codex"]  # AI coding tools — build compression signals
...
def fetch_google_trends():
    ...
    try:
        ...
        # Batch 1: Groups A + B (5 terms — max per pytrends request)
        ...
        # Batch 2: Group C — AI coding tools (separate request)
        if TRENDS_GROUP_C:
...
# (scorecard removed — sidebar simplified)

# ---------------------------------------------------------------------------
# PAGE CONFIG
# ---------------------------------------------------------------------------
st.set_page_config(
    page_title="Software Spend Shift — Evidence Tracker",
    page_icon="📊",
    layout="wide",
...
with tab0:
    ...
    st.caption("Enterprise spend shifting from **70/20/10** (build / maintain / test) → **20/40/40** (build / validate / operate+govern)")
    ...
    if _thesis_scores:
        ...
        _earnings_component = (sum(_earnings_scores) / len(_earnings_scores)) * 10  # 0-10 → 0-100
    else:
    ...
    # Component 5: Search/job trends (0-100) — use Google Trends if cached
    ...
    def _dim_pct(signals, min_signals=3):
        if len(signals) < min_signals:
            # Not enough data — return moderate default scaled by what we have
            if not signals:
                return 40
    ...
    # =======================================================================
    st.markdown("### 📊 Composite Evidence Score")
    ...
    with st.expander("🔧 Score Methodology & Component Weights"):
    ...
    # =======================================================================
    st.markdown("---")
    st.markdown("### 🗺️ Current State Estimate")
    ...
    _est_build = round(70 - (_progress * 50))   # 70 → 20
    _est_validate = round(20 + (_progress * 20)) # 20 → 40
    _est_govern = round(10 + (_progress * 30))   # 10 → 40
    # Normalize to 100
    ...
    with _j_col2:
        ...
        st.markdown(f"""
**Build** (code creation)
<span style="color:#42A5F5; font-size:1.3em;">70% → ~{_est_build}% → 20%</span>

AI coding agents compress build costs. Code written in hours, not weeks.

**Validate** (test & verify)
<span style="color:#66BB6A; font-size:1.3em;">20% → ~{_est_validate}% → 40%</span>

Explosion of software surface area requires massive testing investment.

**Operate & Govern** (run & comply)
<span style="color:#FFA726; font-size:1.3em;">10% → ~{_est_govern}% → 40%</span>
...
""", unsafe_allow_html=True)
    ...
    if _nlp_ts:
        try:
            ...
            _nlp_dot = "🟢" if _nlp_age_h < 24 else ("🟡" if _nlp_age_h < 72 else "🔴")
    ...
    if _all_news:
        ...
        if _news_dates:
            ...
            _news_dot = "🟢" if _news_age_h < 24 else ("🟡" if _news_age_h < 72 else "🔴")
    ...
    if SCORES_FILE.exists():
        ...
        _scores_dot = "🟢" if _scores_age_h < 24 else ("🟡" if _scores_age_h < 72 else "🔴")
...
with tab1:
    ...
    st.caption("Multi-source evidence tracker — 5 intelligence categories, 4-dimension scoring. Articles from scanner_v2 + manual entries.")
    ...
    _CATEGORY_META = {
        "A": ("Enterprise Budget", "🔵", "#2196F3"),
        "B": ("Dev Velocity", "🟢", "#4CAF50"),
        "C": ("Security/Failures", "🔴", "#f44336"),
        "D": ("Compliance", "🟣", "#9C27B0"),
        "E": ("Testing Market", "🟠", "#FF9800"),
    ...
    with col_r1:
        if st.button("🔄 Scan for new articles", key="refresh_news"):
    ...
    with col_r2:
        if NEWS_STORE.exists():
            ...
            st.caption("No scan yet — click Refresh")
    ...
    with st.sidebar:
        ...
        _selected_dims = st.multiselect("Dimension (v2 scored only)", _all_dim_options, default=[], key="intel_dim_filter",
                                         help="Filter by scoring dimension — only applies to v2-scored articles")
    ...
    with col_f1:
        signal_filter = st.selectbox(
            "Signal direction",
            ["All", "🟢 Confirms only", "🔴 Challenges only", "⚪ Neutral only"],
    ...
    keyword_search = st.text_input("🔍 Search articles", "", key="news_keyword")
    ...
    # Apply LLM scores to each article — support both v1 and v2 score schemas
    for a in articles:
        uid = a.get("uid", "")
    ...
    if signal_filter == "🟢 Confirms only":
        filtered = [a for a in filtered if a.get("signal") == "confirms"]
    elif signal_filter == "🔴 Challenges only":
    ...
    sc3.metric("Distribution", f"🟢{n_confirm} 🔴{n_challenge} ⚪{n_neutral}")
    sc4.metric("High-Signal", high_signal, help="Relevance ≥ 4 AND |composite| ≥ 10")
    ...
    if _cat_counts:
        ...
        if _uncategorized_count > 0:
            st.caption(f"📝 Showing categorized articles only. {_uncategorized_count:,} uncategorized articles from legacy scans excluded from chart.")
    ...
    if _dated_scored:
        ...
        if not _ts_df.empty:
            ...
            st.subheader("📈 Evidence Strength Over Time")
    ...
    if _v2_dated:
        ...
        if _mom_rows:
            ...
            st.subheader("🚀 Weekly Signal Momentum (Positive-Direction, by Dimension)")
    ...
    if _dated_scored and not _ts_df.empty:
        ...
        _wc3.metric("Confirm / Challenge", f"🟢{_tw_confirms} / 🔴{_tw_challenges}")
    ...
    if _top3:
        st.subheader("🏆 Top Signal Articles (Last 14 Days)")
        for _t3 in _top3:
            ...
            _t3_dim_str = ", ".join(_t3_dims) if _t3_dims else "—"
    ...
    for a in page_articles:
        ...
        _xlate_line = f"  \n🌐 _{_en_title}_" if _en_title else ""

        if is_irrelevant:
            with st.expander(f"~~{icon} {date_str} · {source} — {a['title']}~~ (irrelevant)", expanded=False):
...
with tab2:
    ...
    st.caption(
        ...
        "Dev Tools basket (GTLB, TEAM, FROG, CFLT) — revenue growth, returns, and divergence"
    )

    # --- Period selector and weighting toggle in sidebar ---
    with st.sidebar:
        st.markdown("### 📈 Company Signal Controls")
    ...
    if data_loaded and _failed_tickers:
        ...
        st.warning(
            f"⚠️ Basket data incomplete: fetched {_fetched_count}/{_total_tickers} tickers. "
            f"Failed: {', '.join(_failed_tickers)}. Returns may be skewed."
        )

    if data_loaded and not fundamentals_df.empty:
        # Revenue growth comparison
        st.subheader("📊 Revenue Growth Comparison (YoY)")
        ...
        st.subheader(f"📈 Basket Cumulative Returns — {_period_option} ({_weight_label})")
...
# Keywords to search for across transcripts — grouped by theme
KEYWORD_GROUPS = {
...
with tab3:
    ...
    if not all_transcripts:
        ...
        st.markdown(f"**📚 {transcript_count} earnings call transcripts across {company_count} companies** (from {total_all} total Koyfin transcripts)")
        ...
        st.subheader("📈 Keyword Mention Trends Across Quarters")
        ...
        if nlp_cache and nlp_cache.get("thesis_scores"):
            ...
            if _nlp_processed_at:
                try:
                    ...
                    if _nlp_age_hours < 24:
                        _nlp_freshness_color = "#4CAF50"  # green — fresh
                    elif _nlp_age_hours < 72:
                        _nlp_freshness_color = "#FF9800"  # orange — ageing
                    else:
                        _nlp_freshness_color = "#f44336"  # red — stale
                except Exception:
                    pass
            ...
            st.markdown(
                ...
                f'<span style="color:{_nlp_freshness_color};">●</span> NLP cache: {_nlp_freshness_str}</span>'
                f'</div>',
                unsafe_allow_html=True,
            )

            # 1. Sentiment Breakdown — per-quarter view (more intuitive than aggregate)
            ...
            with nlp_col1:
                ...
                if quarter_sentiment:
                    ...
                    _sent_q_range = f"{VALID_CY_QUARTERS[0]} – {VALID_CY_QUARTERS[-1]}" if len(VALID_CY_QUARTERS) > 1 else VALID_CY_QUARTERS[0] if VALID_CY_QUARTERS else ""
            ...
            with nlp_col2:
                ...
                if guidance_totals["bullish"] > guidance_totals["bearish"] * 2:
                    st.success(f"🟢 Management language strongly supports thesis ({guidance_totals['bullish']} bullish vs {guidance_totals['bearish']} bearish)")
                elif guidance_totals["bearish"] > guidance_totals["bullish"]:
                    st.warning(f"🟡 More bearish than bullish management language detected")
            ...
            if quarterly_totals and len(quarterly_totals) >= 2:
                st.markdown("---")
                st.subheader("📈 Q-over-Q Keyword Acceleration")
                accel = qoq_data.get("acceleration", {})
                trend_emoji = {"accelerating": "🚀", "decelerating": "📉", "stable": "➡️"}.get(accel.get("overall_trend", ""), "❓")
                ...
                # Build stacked area chart by keyword group — show ALL quarters for trend visibility
                qoq_rows = []
                ...
                if qoq_rows:
                    ...
                    # Sort quarters chronologically (e.g. "CY Q2 2025" → (2025, 2))
            ...
            if thesis_scores:
                st.markdown("---")
                st.subheader("🏆 Company Thesis Ranking")
                st.caption("Composite score (0-10) based on sentiment analysis, management guidance, and keyword trends — ranked strongest → weakest")
                ...
                for _rank_idx, r in enumerate(rankings, 1):
                    ...
                    ranking_rows.append({
                        ...
                        "⬆ Inc": r.get("sentiment_breakdown", {}).get("INCREASING", 0) + r.get("sentiment_breakdown", {}).get("CUSTOMER_DEMAND", 0),
                        "⬇ Dec": r.get("sentiment_breakdown", {}).get("DECREASING", 0),
                ...
                with st.expander("🔍 Top 5 Company Details — Guidance Signals & Key Quotes"):
                    for r in rankings[:5]:
                        _t = r["ticker"]
                        _co = r.get("company", _t)
                        _dir_icon = {"strongly_confirming": "🟢🟢", "confirming": "🟢", "neutral": "⚪", "challenging": "🔴", "strongly_challenging": "🔴🔴"}.get(r.get("direction", ""), "❓")
                        st.markdown(f"**{_dir_icon} {_t}** — {_co} · Score: **{r['thesis_score']}/10**")

                        _gc = r.get("guidance_counts", {})
                        _sb = r.get("sentiment_breakdown", {})
                        _detail_cols = st.columns(3)
                        _detail_cols[0].metric("🟢 Bullish Guidance", _gc.get("bullish", 0) + _gc.get("customer_signal", 0))
                        _detail_cols[1].metric("🔴 Bearish Guidance", _gc.get("bearish", 0))
                        _detail_cols[2].metric("📊 Total Snippets", _sb.get("INCREASING", 0) + _sb.get("DECREASING", 0) + _sb.get("CUSTOMER_DEMAND", 0) + _sb.get("NEUTRAL", 0))
            ...
            # 5. Key Quotes Panel — all available NLP-extracted quotes
            all_quotes = []
            ...
            if all_quotes:
                ...
                st.caption("Most impactful quotes from earnings calls (AI-extracted, confidence ≥ 3/5)")

                # Sort by confidence, then sentiment value
                # Remap CUSTOMER_DEMAND → INCREASING for display
                for q in all_quotes:
                ...
                for i, q in enumerate(all_quotes[:12]):
                    col = quote_cols[i % 2]
                    sent_icon = {"INCREASING": "📈", "DECREASING": "📉", "NEUTRAL": "⚪"}.get(q["sentiment"], "⚪")
                    ...
                    with col:
                        st.markdown(
                            ...
                            f'<span style="color: {sent_color};">{q["sentiment"]}</span> · ⭐{q["confidence"]}/5<br>'
            ...
            st.info("💡 **NLP analysis not yet available.** Run `python earnings_nlp.py` to generate AI-powered sentiment analysis across sample transcripts.")

        # ------- RELEVANT COMMENTS: Latest earnings calls on QA/testing -------
        st.subheader("💬 What Companies Are Saying About Testing & QA")
        st.caption("Relevant snippets from the most recent earnings call for each company — ranked by total testing/governance keyword mentions")
        ...
        if valid_transcripts:
            ...
            if scored:
                ...
                for mention_count, ticker, t in scored[:15]:
                    ...
                    with st.expander(f"**{ticker}** — {company} · {edate} · {mention_count} mentions"):
        ...
        if search_kw:
            ...
            for r in results[:15]:
                with st.expander(f"**{r['ticker']}** — {r['title']} ({r['count']} mentions)"):
        ...
        if quotes:
            for q in quotes:
                ...
                st.markdown(
                    ...
                    f'{icon} <strong>{q["speaker"]}</strong>, {q["company"]} — {q["quarter"]}<br>'
                    f'<em>&ldquo;{q["quote"]}&rdquo;</em>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
            ...
            if _auto_quotes_cache:
                ...
                if _auto_quotes:
                    st.caption("Auto-extracted from earnings calls via NLP (confidence ≥ 4/5). Use the form below to add curated quotes.")
                    for _aq in _auto_quotes[:8]:
                        _aq_icon = {"INCREASING": "📈", "DECREASING": "📉", "NEUTRAL": "⚪"}.get(_aq["sentiment"], "⚪")
                        ...
                        st.markdown(
                            ...
                            f'<span style="color: {_aq_color};">{_aq["sentiment"]}</span> · ⭐{_aq["confidence"]}/5<br>'
...
with tab4:
    ...
    if deals:
        ...
        for _, row in filtered_deals.iterrows():
            ...
            _verification_badge = ""
            if _unverified:
                _verification_badge = " · ⚠️ _unverified_"
            _source_link = ""
            if _source_url:
        ...
        if not funded.empty:
            ...
            if not mega_deals.empty:
                for _, mega in mega_deals.iterrows():
                    ...
                    st.info(f"🐋 **{label}: ${mega['amount_m']:,.0f}M** — excluded from chart for scale. {mega.get('description', '')}")
            ...
            if not chart_deals.empty:
                ...
                st.caption("All deals are mega-deals — no chart to display at normal scale.")
    else:
...
with tab5:
    ...
    if "_error" in trends_data:
        ...
        if trend_rows:
            ...
            # Individual terms
            st.subheader("📈 Individual Search Terms")
            fig_terms = px.line(
            ...
            # Group averages
            st.subheader("📊 Group Average Comparison")
            ...
            st.info("💡 Tip: Try refreshing in a few minutes. Data is cached for 4 hours once fetched.")

# ========================  TAB 6: INCIDENT TRACKER  ========================
with tab6:
    st.header("🚨 Incident Tracker")
    st.caption("Production incidents & security failures — Leapwork's strongest sales catalyst: 'AI-generated code is breaking things, you need testing'")
    ...
    if _total_incidents == 0:
        ...
        st.caption("Category C articles over time — color-coded by governance relevance (GR score)")
        ...
        if not _timeline_df.empty:
            ...
            if len(_weekly_counts) >= 3:
                ...
                fig_timeline.add_trace(go.Scatter(
                    ...
                    y=_trend_y,
                    mode="lines",
                    name=f"Trend ({'↑ increasing' if _coeffs[0] > 0 else '↓ decreasing'})",
        ...
        st.subheader("🔥 Incident Intensity Heatmap")
        st.caption("Monthly incident density — brighter = more incidents")
        ...
        if not _heatmap_df.empty:
            ...
            if not _spikes.empty:
                ...
                st.info(f"📈 **Spike months** (>1.5× average of {_avg_count:.0f}): {', '.join(_spike_strs)}")

        # === 3. AI-Attribution Analysis ===
        st.subheader("🤖 AI-Attribution Analysis")
        ...
        if not _ai_df.empty:
            ...
            if len(_ai_monthly) >= 3:
                ...
                if _delta > 2:
                    st.success(f"📈 AI-attribution rate is **increasing**: {_first_half:.1f}% → {_second_half:.1f}% (+{_delta:.1f}pp)")
                elif _delta < -2:
                    st.warning(f"📉 AI-attribution rate is **decreasing**: {_first_half:.1f}% → {_second_half:.1f}% ({_delta:.1f}pp)")
                else:
                    st.info(f"➡️ AI-attribution rate is **stable**: ~{_ai_monthly['ai_pct'].mean():.1f}%")

        # === 4. Pain Index ===
        st.subheader("💢 Pain Index")
        st.caption("Composite metric: Incident Count × Avg Severity (GR Score) × AI-Attribution Rate — the single most compelling chart for Leapwork's narrative")
        ...
        if not _pain_df.empty:
            ...
            # Pain Index = count × (1 + avg_gr/10) × (1 + ai_rate)
        ...
        st.caption("Category C articles — security failures, production incidents, outages")
        ...
        if _table_data:
            ...
            for a in _table_data[:100]:
                ...
                _display_title = f"{_raw_title}\n🌐 {_en}" if _en else _raw_title
                _table_rows.append({
                    "Date": a["date"][:10] if a["date"] else "?",
                    "Title": _display_title,
                    "Source": a["source"],
                    "GR Score": a["gr_score"],
                    "AI-Related": "🤖 Yes" if a["ai_related"] else "No",
            ...
            st.caption(f"Showing top 100 of {len(_table_data)} incidents. Most articles are unscored (GR=0) — run the v2 scorer to populate scores.")
        else:
...
with tab7:
    ...
    st.subheader("📈 Google Trends — Testing vs Building")
    ...
    if _trends_available:
        ...
        if _trend_rows_li:
            ...
            if "Testing / QA" in _ratio_df.columns and "Dev Tools" in _ratio_df.columns:
                ...
                if not _ratio_df.empty:
                    ...
                    if _latest_ratio > 1:
                        st.success(f"📈 Testing search interest **exceeds** dev tools interest (ratio: {_latest_ratio:.2f})")
                    else:
        ...
        st.info("💡 Terms tracked: 'Agentic AI testing', 'Agentic software development' vs 'Agentic QA', 'AI agent testing', 'AI agent validation' + 'Claude Code', 'ChatGPT Codex'")
    ...
    st.subheader("💰 Funding Pipeline — Testing/QA/Governance")
    st.caption("VC funding flowing into the testing & validation space — timeline and running total")
    ...
    if _funding_deals:
        ...
        _fund_chart["verified_label"] = _fund_chart["unverified"].apply(
            lambda x: "⚠️ Unverified" if x else "✅ Verified"
        )
        ...
        with _fc1:
            ...
            if not _fund_chart.empty:
                fig_fund = px.bar(
                    ...
                    labels={"amount_m": "Deal Value ($M)", "date": ""},
                    color_discrete_map={"✅ Verified": "#81C784", "⚠️ Unverified": "#FFB74D"},
                )
        ...
        _fm3.metric("Verification", f"✅{_verified_count} / ⚠️{_unverified_count}")
    else:
    ...
    st.subheader("📊 Market Sizing Estimates")
    st.caption("Key market size estimates from analyst reports — how big is the opportunity?")
    ...
    if _market_data:
        ...
        if not _mkt_current.empty and not _mkt_forecast.empty:
            ...
            if _pairs:
                ...
                _tc3.metric("Growth Multiple", f"{_tam_forecast/_tam_current:.1f}×" if _tam_current > 0 else "—")

        # Full data table
        with st.expander("📋 All Market Sizing Data"):
            st.dataframe(
        ...
        st.info("ℹ️ Market sizing data is manually curated. Update `data/market_sizing.json` with new estimates as they become available.")
    else:
